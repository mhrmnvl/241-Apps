...
vi.mock('@/features/academic/academic-calendar', () => ({
  academicCalendarApi: { getCalendars: vi.fn() },
}))
...
const { academicCalendarApi } =
  await import('@/features/academic/academic-calendar')
...
describe('calendar import preview', () => {
  ...
  it('expands a multi-day entry into one row per date', async () => {
    vi.mocked(academicCalendarApi.getCalendars).mockResolvedValue({
      data: {
        data: [
          {
            id: 'cal-1',
  ...
  it('clears the preview when the calendar cannot be read', async () => {
    ...
    vi.mocked(academicCalendarApi.getCalendars).mockRejectedValue(
      new Error('boom'),
    )
