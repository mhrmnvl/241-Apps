...
export const workPatternService = {
   ...
   * would close a domain cycle (research R9). The preview is the bonus — nobody
   * changes what counts as a working day without seeing the list.
   */
  previewFromCalendar: async (academicYearId: string, typeId: string) => {
    try {
      const res = await academicCalendarApi.getCalendars({
        academicYearId,
        typeId,
