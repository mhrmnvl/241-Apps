...
function formatErrorMessage(error?: string): string {
  ...
  // Hapus duplikasi baris jika ada (misal setelah identifier diubah jadi NIS)
...
function renderCellWithError(
  ...
  if (key === 'identifier' || key === 'username') {
    ...
    const detail = getFieldErrorDetail('nis', cleanFullError)
    if (detail) {
      hasError = true
      tooltipText = detail
    }
  } else if (key === 'nisn') {
    const detail = getFieldErrorDetail('nisn', cleanFullError)
    if (detail) {
      hasError = true
      tooltipText = detail
    }
    ...
    const detail = getFieldErrorDetail('grade', cleanFullError)
    if (detail) {
      hasError = true
      tooltipText = detail
    }
  } else if (key === 'classroomcode') {
    const detail = getFieldErrorDetail('classroom', cleanFullError)
    if (detail) {
      hasError = true
      tooltipText = detail
    }
...
const columns = computed<ColumnDef<BulkImportRowResult>[]>(() => {
  ...
  return [
    ...
    },
    {
      ...
      meta: { align: 'center' },
      cell: ({ row }) =>
        renderCellWithError(row.original, 'nis', row.original.data?.nis),
    },
    {
      id: 'nisn',
      header: 'NISN',
      meta: { align: 'center' },
      cell: ({ row }) =>
        renderCellWithError(row.original, 'nisn', row.original.data?.nisn),
    },
    {
      id: 'name',
      header: 'Nama',
    ...
    },
    {
      ...
      meta: { align: 'center' },
      cell: ({ row }) =>
        renderCellWithError(row.original, 'grade', row.original.data?.grade),
    },
    {
      id: 'classroomCode',
      header: 'Kelas',
      ...
      cell: ({ row }) =>
        renderCellWithError(
          row.original,
          'classroomCode',
          row.original.data?.classroomCode,
        ),
    },
    {
      id: 'identifier',
