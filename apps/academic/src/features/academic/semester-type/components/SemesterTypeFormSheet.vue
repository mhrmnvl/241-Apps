...
import { useSemesterTypeForm } from '../composables/useSemesterType'
...
const { isSubmitting, createSemesterType, updateSemesterType } =
  useSemesterTypeForm()
...
const formSchema = toTypedSchema(
  z.object({
    name: z
      .string()
      .min(1, 'Tipe semester wajib diisi.')
      .max(50, 'Tipe semester maksimal 50 karakter.'),
...
const onSubmit = handleSubmit(async (values) => {
  const success = props.initialData
    ? await updateSemesterType(props.initialData.id, values)
    : await createSemesterType(values)
  ...
  >
    <SheetContent class="w-full sm:max-w-md flex flex-col gap-0 border-l p-0">
      <SheetHeader class="px-6 py-6 border-b shrink-0 bg-muted/20">
        <SheetTitle
          >{{ initialData ? 'Edit' : 'Tambah' }} Tipe Semester</SheetTitle
        >
        <SheetDescription>
          {{
            initialData
