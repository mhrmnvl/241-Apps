...
const { isSubmitting, createEmploymentType, updateEmploymentType } =
...
const formSchema = toTypedSchema(
  z.object({
    code: z
      .string()
      .min(1, 'Kode status wajib diisi.')
      .max(20, 'Kode status maksimal 20 karakter.'),
    name: z
      .string()
...
const onSubmit = handleSubmit(async (values) => {
  const success = props.initialData
    ? await updateEmploymentType(props.initialData.id, { name: values.name })
  ...
  >
    <DialogContent class="sm:max-w-md flex flex-col gap-0 p-0 overflow-hidden">
      <DialogHeader class="px-6 py-5 border-b shrink-0 bg-muted/20">
        <DialogTitle
          >{{ initialData ? 'Edit' : 'Tambah' }} Status Kepegawaian</DialogTitle
        >
        <DialogDescription>
          {{
            initialData
              ? 'Perbarui data status kepegawaian.'
              : 'Tambahkan status kepegawaian baru ke dalam sistem.'
          }}
        </DialogDescription>
      </DialogHeader>
      ...
      <ScrollArea class="flex-1 min-h-0">
        ...
        >
          ...
          >
            <FormItem>
              <FormLabel
                >Kode Status <span class="text-destructive">*</span></FormLabel
              >
              <FormControl>
                <Input
