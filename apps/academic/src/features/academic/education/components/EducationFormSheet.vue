...
const { isSubmitting, createEducationLevel, updateEducationLevel } =
  useEducationForm()
...
const formSchema = toTypedSchema(
  z.object({
    name: z
      ...
      .max(100, 'Nama tingkat pendidikan maksimal 100 karakter.'),
...
const onSubmit = handleSubmit(async (values) => {
  const success = props.initialData
    ? await updateEducationLevel(props.initialData.id, values)
    : await createEducationLevel(values)
  ...
  >
    <SheetContent class="w-full sm:max-w-md flex flex-col gap-0 border-l p-0">
      <SheetHeader class="px-6 py-6 border-b shrink-0 bg-muted/20">
        <SheetTitle
          >{{ initialData ? 'Edit' : 'Tambah' }} Tingkat Pendidikan</SheetTitle
        >
        <SheetDescription>
          {{
            initialData
