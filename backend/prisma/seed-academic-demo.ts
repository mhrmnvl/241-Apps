...
import { GenerateReportCardUseCase } from '../src/academic/report-card/use-cases/generate-report-card.use-case.js';
 ...
 *
 * The report cards are generated by `GenerateReportCardUseCase` rather than
 * written here. A fixture that inserts its own rows proves only that rows can
 * be inserted — the existing two students had a rapor with an average, a rank,
 * and no subject lines at all, because nothing had ever run the real
 * calculation for them. Booting the application to call it is the only way the
 * demo shows what production would.
 *
...
async function main() {
  ...
  // 6. The rapor, through the code that makes rapor.
  //
  // A published card refuses regeneration on purpose — it has been handed to a
  // parent. Here it is unpublished first, deliberately, which is the same act
  // the screen requires of a person.
  const app = await NestFactory.createApplicationContext(AppModule, {
    logger: ['error'],
  });
  const generate = app.get(GenerateReportCardUseCase);
  ...
  for (const enrolment of allEnrolments) {
    ...
    const card = await generate.execute({ enrollmentId: enrolment.id });
    generated.push({
      enrollmentId: enrolment.id,
      classroomId: enrolment.classroomId,
      average: card.totalAverage ?? 0,
    });
  }
