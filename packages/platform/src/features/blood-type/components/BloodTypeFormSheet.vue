...
import { useBloodTypeForm } from '../composables/useBloodTypeForm'
...
const { isSubmitting, createBloodType, updateBloodType } = useBloodTypeForm()
...
const formSchema = toTypedSchema(
  z.object({
    name: z
      ...
      .max(100, 'Nama golongan darah maksimal 100 karakter.'),
...
const onSubmit = handleSubmit(async (values) => {
  const success = props.initialData
    ? await updateBloodType(props.initialData.id, values)
    : await createBloodType(values)
  ...
  >
    <SheetContent class="w-full sm:max-w-md flex flex-col gap-0 border-l p-0">
      <SheetHeader class="px-6 py-6 border-b shrink-0 bg-muted/20">
        <SheetTitle
          >{{ initialData ? 'Edit' : 'Tambah' }} Golongan Darah</SheetTitle
        >
        <SheetDescription>
          {{
            initialData
